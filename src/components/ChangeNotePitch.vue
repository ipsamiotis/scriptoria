<template>
    <BlockUI :blocked="state.blockedPanel">
        <div class="button-group">
            <h4>Pitch: {{state.noteDurations[state.sliderValue]}}</h4>
            ...
            <div class="note-rest">
                <Button id="noteBtn" type="button" label="Note" :badge="state.numNotes" class="p-button-outlined p-button-secondary" @click="addMusicElement($event)"/>
                <Button id="restBtn" type="button" label="Rest" :badge="state.numRests" class="p-button-outlined p-button-secondary" @click="addMusicElement($event)"/>
            </div>
            <InlineMessage v-if="state.noteMsg && state.sliceElements.length==0" severity="warn">Please add an element</InlineMessage>
        </div>
    </BlockUI>
    <Button id="submitButton" type="button" label="Completed Sequence" :icon="state.blockedPanel ? 'pi pi-check-circle' : 'pi pi-circle-off'" @click="blockAndSubmit(state.sliceElements)"/>
</template>
